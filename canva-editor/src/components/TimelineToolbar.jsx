...
import TimelineItemsEditor from "./TimelineItemsEditor";
...
export default function TimelineToolbar({ el }) {
  const update = (props) =>
  ...
  return (
    <div className="h-12 bg-white border-b border-gray-200 flex items-center px-4 gap-2 overflow-x-auto">
      ...
      <ToolBtn
        ...
        title="Edit timeline items"
      >
        <List className="w-4 h-4" />
        <span className="text-xs ml-1 font-medium">Edit Items</span>
      </ToolBtn>
      ...
      {showItemsEditor && (
        <TimelineItemsEditor
          el={el}
          onClose={() => setShowItemsEditor(false)}
        />
      )}
    </div>
  );
