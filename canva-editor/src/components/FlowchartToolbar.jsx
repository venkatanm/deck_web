...
const SelectInput = ({ value, onChange, options }) => (
  <select
    ...
    className="w-28 text-xs border border-gray-200 rounded-md px-2 py-1 focus:outline-none focus:border-purple-400 bg-white cursor-pointer"
  >
    {options.map((o) => (
...
const NumberInput = ({ label, value, onChange, min, max }) => (
  <div className="flex items-center gap-1 flex-shrink-0">
    ...
    <input
      ...
      className="w-12 text-xs border border-gray-200 rounded-md px-2 py-1 text-center focus:outline-none focus:border-purple-400"
    />
  </div>
);
...
export default function FlowchartToolbar({ el }) {
  const update = (props) =>
    useEditorStore.getState().updateElement(el.id, props);
  const startConnection = () => {
    useEditorStore.setState({ connectionFromId: el.id });
  };
  ...
  return (
    <div className="h-12 bg-white border-b border-gray-200 flex items-center px-4 gap-2 overflow-x-auto">
      ...
      <ToolBtn onClick={startConnection} title="Start connector from this shape">
        <Link2 className="w-4 h-4" />
        <span className="text-xs ml-1">Connect</span>
      </ToolBtn>
      ...
      <ColorPickerBtn
        color={el.fill || "#ede9fe"}
        onChange={(v) => update({ fill: v })}
        title="Fill"
      />
      <ColorPickerBtn
        color={el.stroke || "#7c3aed"}
